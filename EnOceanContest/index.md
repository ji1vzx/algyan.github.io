...
- 「EnOceanとKNXで実現する」はマルチセンサーの状態によりKNX経由でカラーLEDを制御する面白い作品です。実質的にNode-REDで実装した EnOcean - KNX ゲートウェイの紹介がメインとなり、マルチセンサーは点灯LED切り替えスイッチの役割です。欧米の制御分野ではEnOcean - KNX ゲートウェイは結構普及して実用化されているため、これも評価は厳しくなりました。
